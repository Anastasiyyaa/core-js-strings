...
function removeFirstOccurrences(str, value) {
  const starIndex = str.indexOf(value);
  ...
  elements.splice(starIndex, value.length);
...
function removeLastOccurrences(str, value) {
  const starIndex = str.lastIndexOf(value);
...
function orderAlphabetically(str) {
  const reversedString = str.split('').sort().join('');
  return reversedString;
}
...
function countVowels(str) {
  ...
  let sum = 0;
  for (let i = 0; i < strupp.length; i += 1) {
    if (
      strupp[i] === 'A' ||
      strupp[i] === 'E' ||
      strupp[i] === 'I' ||
...
function encodeToRot13(str) {
  return str
    .split('')
    .map((input) => {
      const idx = cipherInput.indexOf(input);

      return idx === -1 ? input : cipherOutput[idx];
    })
    .join('');
}
...
function getCardId(value) {
  return CARD_NUMBER.indexOf(value);
}
